        ...
        <el-col :span="24" class="main">
          <aside :class="collapsed? 'menu-collapsed':'menu-expanded'">
            ...
            <el-menu background-color="#f1f2f7" :default-active="$route.path"  class="el-menu-vertical-demo"  @open="handleOpen" @close="handleClose"  @select="handleselect" unique-opened router v-show="!collapsed">
              <!--一级菜单的名字为路由过来组件的路径名-->
              <template v-for="(item,index) in $router.options.routes" v-if="item.sell">
                <el-submenu :index="index+''"  v-if="!item.leaf">
                  <template slot="title" ><i :class="item.iconCls"></i>{{item.name}}</template>
                  <!--二级菜单的名字为子路由的名字-->
                  <el-menu-item v-for="child in item.children" :index="child.path" :key="child.path" v-if="!child.sell">{{child.name}}</el-menu-item>
                </el-submenu>
            ...
            <ul class="el-menu  collapsed" v-show="collapsed" ref="menuCollapsed">
              <li v-for="(item,index) in $router.options.routes" v-if="item.sell" class="el-submenu item">
                <!--如果子菜单不是单一节点-->
                <template v-if="!item.leaf">
                  <div class="el-submenu__title" style="padding-left: 20px;" @mouseover="showMenu(index,true)" @mouseout="showMenu(index,false)"><i :class="item.iconCls"></i></div>
                  <ul class="el-menu submenu" :class="'submenu-hook-'+index" @mouseover="showMenu(index,true)" @mouseout="showMenu(index,false)">
                    <li v-for="child in item.children" v-if="!child.sell" :key="child.path" class="el-menu-item" style="padding-left: 20px;" :class="$route.path===child.path?'is-active':''" @click="$router.push(child.path)">{{child.name}}</li>
                  </ul>
                </template>
                <!--如果子菜单是单一节点-->
                <template v-else>
...
<script>
    export default {
      ...
      methods:{
        ...
        introduce(){
          this.$router.push('/homepage1/introduce1');
        },
